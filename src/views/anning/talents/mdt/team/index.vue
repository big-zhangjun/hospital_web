                    ...
                    <el-row>
                        ...
                        <el-col :span="24">
                            <el-form-item label="服务地区" prop="serviceArea">
                                <el-cascader v-model="queryParams.serviceArea" :options="provinceList" :props="props"
...
import { getMdtTeamlist, addMdtTeam, getMdtTeam, updateMdtTeam, delMdtTeam } from "@/api/anning/talents/hospital/team.js";
    ...
    data() {
        return {
            ...
            rules: {
                ...
                captainDoctorId: [{ required: true, message: "队长不能为空", trigger: "blur" }],
    ...
    methods: {
        ...
        submitForm: function () {
            this.$refs["form"].validate((valid) => {
                if (valid) {
                    let members = this.docList.filter(item => this.form.members.includes(item.id)).map(item => {
                        return {
                            id: item.id,
                            name: item.name
                        }
                    })
                    // let serviceArea = this.form
                    if (this.type == 'edit') {
                        updateMdtTeam({ ...this.form, members }).then((response) => {
                        ...
                        addMdtTeam({ ...this.form, members }).then((response) => {
